...
CREATE TABLE IF NOT EXISTS audit_records (
    ...
    context VARCHAR(16) NOT NULL
        CHECK (context IN ('INNIE', 'OUTIE', 'SYSTEM', 'ADMIN')),
    task_type VARCHAR(64) NOT NULL,
    final_state VARCHAR(32) NOT NULL
        CHECK (final_state IN ('COMPLETED', 'ARCHIVED', 'PURGED', 'FAILED')),
...
CREATE INDEX IF NOT EXISTS idx_audit_compartment_id
    ON audit_records(compartment_id);
CREATE INDEX IF NOT EXISTS idx_audit_owner_id
    ON audit_records(owner_id);
